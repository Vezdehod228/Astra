...
VOID CDraw::InitFonts()
{
	...
	g_pSurface->SetFontGlyphSet(m_WatermarkFont, /*Calibri*/XorStr<0x83, 16, 0x5F987AA5>("\xD5\xED\xF6\xEF\xF3\xE7\xFB\xAA\xDF\xD8\xBF\xAE\xCD\xC2\xDA" + 0x5F987AA5).s, 16, 500, 0, 0, 0x050);
	...
	nameESP = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(nameESP, "RanyLight", 14, 400, 0, 0, 0x010);

	dropDownFont = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(dropDownFont, "RanyLight", 17, 400, 0, 0, 0x010);

	m_ListItemFont = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(m_ListItemFont, "RanyLight", 16, 400, 0, 0, 0x010);

	Testing = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(Testing, "RanyLight", 18, 425, 0, 0, 0x010);

	NameCheatFont = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(NameCheatFont, "RanyLight", 18, 445, 0, 0, 0x010);

	ForMiniTabPanel = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(ForMiniTabPanel, /*Visitor TT2 BRK*/XorStr<0x83, 16, 0x5F987AA5>("\xD5\xED\xF6\xEF\xF3\xE7\xFB\xAA\xDF\xD8\xBF\xAE\xCD\xC2\xDA" + 0x5F987AA5).s, 16, 500, 0, 0, 0x010);

	ForTabPanel = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(ForTabPanel, "RanyLight", 17, 425, 0, 0, 0x010);

	TabPanelFont = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(TabPanelFont, "RanyLight", 15, 450, 0, 0, 0x010);

	ForCheckbox = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(ForCheckbox, "RanyLight", 18, 500, 0, 0, 0x010);
}
