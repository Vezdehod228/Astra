...
void cGameEventManager::FireGameEvent(ValveSDK::IGameEvent *event)
{
	...
	static std::string roundStart = "round_start";
	static std::string killSay = "attacker";
	...
	*/
	...
	if (strcmp(szEventName, sXor.data()) == 0)
	{
		...
		if (event->GetInt(killSay.data(), 0) - 1 == g_Valve.pEngine->GetLocalPlayer())
		{
	...
	if (strcmp(szEventName, roundStart.data()) == 0)
	{
		g_healthInfo.clear();
	}

	if (szEventName == roundStart)
	{
		Sleep(100);
